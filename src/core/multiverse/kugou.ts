...
// 浏览器扩展全局（async 库）。Node 侧该分支不可达，declare 不产出运行时代码，
// 行为与旧 util/kugou.js 完全一致
// eslint-disable-next-line @typescript-eslint/no-explicit-any
declare const async: any
/* global async getParameterByName */
import { axios, getParameterByName, changeUrlQuery } from './base'
export class kugou {
  static api_map = {
    "/search" : kugou.search,
    "/playlist/detail": kugou.kg_get_playlist,
    // "/top/playlist": kugou.qq_show_toplist,
    // "/related/playlist":qq.show_playlist,
    // "/song/detail": kugou.get_song_detail,
    // "/song/url/v1": kugou.get_mp3_url,
    // "/song/url": kugou.get_mp3_url,
    "/lyric": kugou.lyric,
  }
  static kg_convert_song(song) {
    const track = {
      ...
      ar: song.singer,
      playable: (song.pay_type==0||song.pay_type_320==0||song.pay_type_sq==0)?1:0,
      al: {name:song.album_name},
      // picUrl: kugou.qq_get_image_url(song.albummid, 'album'),
      source: 'kugou',
      sourceUrl: `https://www.kugou.com/song/#hash=${song.hash}&album_id=${song.album_id}`,
      url: `https://api.wuenci.com/meting/api/?type=url&id=${song.hash}&server=kugou`,
    };
  ...
  static kg_convert_playlist(item) {
    const track = {
      ...item,
      id: `${item.specialid}`,
      name: item.specialname,
      source: 'kugou',
      sourceUrl: 'https://www.kugou.com/yy/special/single/{size}.html'.replace(
          '{size}',
          item.specialid
      ),
      picUrl: item.imgurl
  ...
  static search(url) {
    const keyword = getParameterByName('keywords', url) || "群青";
    const curpage = getParameterByName('curpage', url) || 1;
    const searchType = getParameterByName('type', url) || "1000";
    const typeTable = {
      "1000":"special",
      "1":"song"
    }
    if (searchType) {
      return {
        success: (fn) => {
          const target_url = `http://mobilecdnbj.kugou.com/api/v3/search/${typeTable[searchType]}?keyword=${keyword}&pagesize=20&filter=0&page=${curpage}`;
          axios.get(target_url)
              .then((response) => {
                ...
                // });
                  ...
                  if(searchType == "1"){
                    res.songs = response.data.data.info.map((item) => 
                      kugou.kg_convert_song(item)
                    );
                  }
                  if(searchType == "1000"){
                    res.playlists = response.data.data.info.map((item) => kugou.kg_convert_playlist(item));
                  }
  ...
  static kg_get_playlist(url) {
    return {
      success: (fn) => {
        const list_id = getParameterByName('id', url).split('_').pop();
        const target_url = `https://m.kugou.com/plist/list/${list_id}?json=true`;
        // const target_url = `http://mobilecdnbj.kugou.com/api/v3/rank/list?id=${list_id}&version=9108&plat=0&showtype=2&parentid=0&apiver=6&area_code=1&withsong=1&with_res_tag=1`
        axios.get(target_url).then((response) => {
          const { data }  = response;
          console.log(data);
          return fn({
  ...
  static show_playlist(url) {
    let offset: any = getParameterByName('offset', url);
    if (offset === undefined) {
      offset = 0;
    }
